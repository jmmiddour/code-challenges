...
class LogTransferTests(unittest.TestCase):
    ...
    def test_float(self):
      ...
      self.assertEqual(log_transfer(.006), "Deposited $0.01")
    ...
    def test_exceptions(self):
      with self.assertRaises(TypeError) as context:
        log_transfer(True)
      self.assertEqual(
          str(context.exception).lower(), "x must be a string, int, or float",
      )
      with self.assertRaises(ValueError) as context:
        log_transfer("ALLTHEMONIES")
      self.assertEqual(
          str(context.exception).lower(), "x is not a valid number",
      )
